...
;(function() {
  ...
  var proceedPermanent = function() {
    console.log(getTargetHostname())
    var domain = tldjs.getDomain(getTargetHostname())
  ...
  /******************************************************************************/
  ;(function() {
    var matches = /^(.*)\{\{hostname\}\}(.*)$/.exec(
      vAPI.i18n('docblockedProceed')
    )
    if (matches === null) {
      return
    }
    ...
    if (details.hn === details.dn) {
      ...
      proceed
        .descendants('.hn')
        .text(details.hn)
        .attr('value', details.hn)
      proceed
        .descendants('.dn')
        .text(details.dn)
        .attr('value', details.dn)
    }
  ...
  //   uDom('#proceedTemporary')
  //     .attr('href', details.url)
  //     .on('click', proceedTemporary)
  uDom('#proceedPermanent')
    .attr('href', details.url)
    .on('click', proceedPermanent)
